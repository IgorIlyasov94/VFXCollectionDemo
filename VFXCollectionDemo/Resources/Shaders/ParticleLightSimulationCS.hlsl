...
Texture2D animationTexture : register(t1);

RWStructuredBuffer<Particle> particleBuffer : register(u0);
RWStructuredBuffer<PointLight> particleLightBuffer : register(u1);
...
void main(uint3 dispatchThreadId : SV_DispatchThreadID)
{
	...
	PointLight pointLight = (PointLight)0;
	pointLight.position = particle.position;
	
	float alpha = saturate(particle.life / particle.startLife);
	float4 lightColor = animationTexture.SampleLevel(samplerLinear, float2(alpha, 0.1f), 0.0f);
	
	pointLight.color = lightColor.xyz * lightColor.w * maxLightIntensity;
	
	particleLightBuffer[particleIndex] = pointLight;
}
